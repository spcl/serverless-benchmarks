...
def upload_dataset(bucket, X, y):
    ...
    features = client.upload(bucket, "features.npy", features_path)
    labels = client.upload(bucket, "labels.npy", labels_path)
...
def handler(event):
    ...
    X_key, y_key = upload_dataset(bucket, X, y)

    schedules = [{**c, "features": X_key, "labels": y_key, "bucket": bucket} for c in classifiers]
    return {
        "schedules": schedules
    }
