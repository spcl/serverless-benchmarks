...
def handler(event):
    ...
    data = client.download_within_range(
        benchmark_bucket,
        columns_bucket + "/" + individuals_input,
        start_bytes,
        end_bytes,
    )
    ...
    outputfile_name = client.upload(
        benchmark_bucket,
        individuals_bucket + "/" + outputfile,
        os.path.join("/tmp/", outputfile),
    )
    ...
    return {"individuals_output": outputfile_name}
