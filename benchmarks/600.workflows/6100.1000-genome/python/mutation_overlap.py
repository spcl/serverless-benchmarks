...
def handler(event):
    ...
    client.download(
        benchmark_bucket,
        output_bucket + "/" + individuals_merge_filename,
        individuals_merge_file,
    )
    ...
    mutation_index_array, total_mutations, total_mutations_list = rd.read_individuals(
        ids, rs_numbers, data_dir, chrom, individuals_merge_filename
    )
    ...
    wr.write_pair_individuals(
        random_indpairsfile,
        random_pairs_overlap,
    )
    ...
    tar.add(plots_dir)
    tar.close()
    result_name = client.upload(
        benchmark_bucket,
        output_bucket + "/" + "chr%s-%s.tar.gz" % (c, POP),
        "/tmp/chr%s-%s.tar.gz" % (c, POP),
    )
    result_name = result_name.replace(output_bucket + "/", "")

    return {"output_mutation_overlap": result_name}
