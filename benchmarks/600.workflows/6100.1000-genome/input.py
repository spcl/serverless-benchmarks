...
def generate_input(
    ...
    with open(os.path.join(data_dir, files[0]), "r") as f:
        content = f.readlines()
        # limit content size for local test runs to keep tasks responsive
        content = content[: min(len(content), 500)]
        range_per_job = len(content) / num_individuals_jobs
        for i in range(0, num_individuals_jobs):
            ...
            output = {"start_bytes": start_bytes, "end_bytes": start_bytes + nbytes - 1}
