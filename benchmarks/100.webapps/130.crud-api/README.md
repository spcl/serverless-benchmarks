...
On AWS, the identity running SeBS must be allowed to create, describe, seed, and clean up the benchmark's DynamoDB table. When SeBS resolves its default Lambda execution role, it grants the required item reads, writes, and queries; configured or cached roles must provide them explicitly. See the AWS section of the [platform documentation](../../../docs/platforms.md) for the exact actions.
