...
class GCPGenerator(Generator):
    ...
    def postprocess(self, payloads: List[dict]) -> dict:
        assign_input = {
            "assign_input": {
                "assign": [{"payload": "${input.payload}"}, {"request_id": "${input.request_id}"}]
            }
        }
    ...
    def encode_map(self, state: Map) -> Union[dict, List[dict]]:
        ...
        payload = [
            {state.name + "_init_empty": {"assign": [{array: "${[]}"}]}},
            {
                state.name
                + "_init": {
                    "for": {
            ...
            {
                "assign_payload_"
                + state.name: {"assign": [{"payload." + state.array: "${" + res_name + "}"}]}
            },
        ]
        ...
        if state.common_params:
            ...
            payload[1][state.name + "_init"]["for"]["steps"][0][state.name + "_body"]["assign"][0][
                tmp
            ]["payload"] = entries
                    ...
                    "steps": [
                        {
                            ...
                            + "_body": {
                                ...
                                "args": {
                                    "url": url,
                                    "body": {"request_id": "${request_id}", "payload": "${val}"},
                                },
                            }
                        }
                    ],
    ...
    def __replace_parallel(self, func, func_name, first_func_name):
        if len(func) == 2:
            # task state
            func[1]["assign_payload_" + func_name]["assign"] = [
                {func_name: "${" + first_func_name + ".body}"}
            ]
            func[0][func_name]["result"] = first_func_name
        else:
            # it is a map state
            func[3]["assign_payload_" + func_name]["assign"] = [
                {
                    first_func_name: func[3]["assign_payload_" + first_func_name]["assign"][0][
                        func[1][func_name + "_init"]["for"]["in"]
                        .replace("$", "")
                        .replace("{", "")
                        .replace("}", "")
                    ]
    ...
    def encode_parallel(self, state: Parallel) -> Union[dict, List[dict]]:
        ...
        for i, subworkflow in enumerate(state.funcs):
            ...
            self.__replace_parallel(
                parallel_funcs[-1], parallel_funcs_names[-1], parallel_funcs_names[0]
            )
        ...
        payload = [
            {state.name + "_init": {"assign": dict_names}},
            {
                state.name: {
                    "parallel": {
        ...
        for workflow_id, url in self._map_funcs.items():
            ...
            if len(branch) != 0:
                steps_int += [
                    {
                        "assign_payload_"
                        + workflow_id: {  # {"assign": [{"payload": "${payload.body}"}]}}]
                            "assign": [
            ...
            yield (workflow_id, self._export_func(workflow))
