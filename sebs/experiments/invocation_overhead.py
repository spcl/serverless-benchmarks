...
class InvocationOverhead(Experiment):
    ...
    def run(self):
        ...
        with open(output_file, 'w') as csvfile:
            ...
            writer.writerow(["payload_size", "repetition", "is_cold", "connection_time", "start_timestamp", "finish_timestamp", "request_id"])
            ...
            for pt in pts:
                ...
                benchmark_path = find_benchmark("030.clock-synchronization", "benchmarks")
    ...
    def process(self, directory: str):
        ...
        df['clock_drift'] = ((df['client_send'] - df['server_rcv']) + (df['client_rcv'] - df['server_send'])) / 2
        print(df)

        with open(os.path.join(directory, 'invocation-overhead', 'result.csv')) as csvfile:
            with open(os.path.join(directory, 'invocation-overhead', 'result-processed.csv'), 'w') as csvfile2:
                reader = csv.reader(csvfile, delimiter=',')
                writer = csv.writer(csvfile2, delimiter=',')
                writer.writerow(["payload_size", "repetition", "is_cold", "connection_time", "start_timestamp", "finish_timestamp", "request_id", "clock_drift_mean", "clock_drift_std", "invocation_time"])
                iter2 = iter(reader)
                next(iter2)
                for row in iter2:
                    #print(row)
    ...
    def receive_datagrams(self, input_benchmark: dict, repetitions: int, port: int, ip: str):
        ...
        return [is_cold, conn_time, client_timestamp.timestamp(), server_timestamp, request_id]
