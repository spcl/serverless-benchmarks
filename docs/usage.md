...
to file `out_storage.json`. Memory measurements are taken at an interval of 100ms. A value of -1 will not take any measurements at all. A value of 0 will continuously measure as fast as possible. If the desired interval duration is shorter than a measurement takes to execute, a warning is printed to the console.

```
./sebs.py storage start minio --port 9011 --output-json out_storage.json --measure-interval 100
```
