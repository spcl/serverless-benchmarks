...
class ExperimentEnvironment:
    ...
    def disable_boost(self, cores: List[int]):
        if governor == 'intel_pstate':
            ...
            self._prev_boost_status = execute('cat ' + path)
    ...
    def enable_boost(self, cores: List[int]):
        if governor == 'intel_pstate':
            boost_path = '/sys/devices/system/cpu/intel_pstate'
            execute('echo {status} | sudo tee {path}'.format(
                status=self._prev_boost_status,
                path=boost_path
            ))
        else:
            raise NotImplementedError()
    ...
    def set_frequency(self, max_freq: int):
        ...
        execute('echo {freq} | sudo tee {path}'.format(
            freq=max_freq,
            path=path
        ))
